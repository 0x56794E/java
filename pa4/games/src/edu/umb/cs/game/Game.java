 ...
 * Iterator<Move> instead of older MoveIterator. Also PlayerNumber is now an
 * enum.
 *
 * @author Ethan Bolker
 *
...
public abstract class Game extends Observable
{
    ...
    /**
     * Configure the game before it starts Only needed if needSetup returns true
     * Must be called before init/initObservable (or followed by call to
     * init/initObservable)
     *
     * @param params
     ...
     */
    /**
     * Set up the position ready to play. <p> This should <i>not<\i> be done in
     * the constructor.
     */
     ...
     */
    /**
     * Set up the position ready to play and notify Observers. Note that the
     * first notification signals that the game has just been init'd.
     */
     ...
     * @throws IllegalMoveException if m is illegal.
     */
     ...
     * including whoseTurn(), winner(), and the move list from getMoves. The
     * intention is that two not-over game states are equal if the board
     * position is the same and the player to play next is the same. Concluded
     * game states may lump various board positions together, but must differ
     * based on winner() values.
     */
    @Override
